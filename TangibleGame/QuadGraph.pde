...
boolean validArea(PVector c1, PVector c2, PVector c3, PVector c4, float max_area, float min_area) {
  ...
  //added dividing with img.width*img.height
  float area = Math.abs(0.5f * (i1 + i2 + i3 + i4));
  area = map(area, 0, 1000000, 0, imgproc.img.width*imgproc.img.height);
  println(area + " and max is : " + max_area);
...
Boolean checkQuad(PVector c1, PVector c2, PVector c3, PVector c4){
  ...
  return isConvex(c1, c2, c3, c4) && validArea(c1, c2, c3, c4, imgproc.img.width*imgproc.img.height, imgproc.img.width*imgproc.img.height/1000) && nonFlatQuad(c1, c2, c3, c4);
}

List<PVector> drawQuads(List<PVector> lines){ //<>// //<>//

this.build(lines, imgproc.img.width, imgproc.img.height);
List<PVector> accLines = new ArrayList<PVector>();

for (int[] quad : this.findCycles()) {
  ...
  //we have to add the second condition because it is possible if we increment nLines, to have 2 allowed quads
  if(checkQuad(c12, c23, c34, c41) && accLines.size() < 4){
  ...
  imgproc.linesImg.beginDraw();
  
  imgproc.linesImg.stroke(204,102,0);
  linePVector(l1);
  linePVector(l2);
  linePVector(l3);  
  linePVector(l4);
  
  imgproc.linesImg.endDraw();
...
//we return the lines that create the quad because we want to find the intersection of those, not every line returned by hough transform
return accLines;
}
...
void linePVector(PVector p1){
    ...
    int x0 = 0;
    int y0 = (int) (p1.x / sin(p1.y));
    int x1 = (int) (p1.x / cos(p1.y));
    int y1 = 0;
    int x2 = imgproc.img.width;
    int y2 = (int) (-cos(p1.y) / sin(p1.y) * x2 + p1.x / sin(p1.y));
    int y3 = imgproc.img.width;
    int x3 = (int) (-(y3 - p1.x / (sin(p1.y)) * (sin(p1.y) / cos(p1.y))));
       
     println(x0 + " " + y0 + " " + x1 + " " + y1 + " " + x2 + " " + y2 + " " + y3 + " " + x3);
     
    if (y0 > 0) {
    if (x1 > 0){
    imgproc.linesImg.line(x0, y0, x1, y1);
    }
    else if (y2 > 0){
    imgproc.linesImg.line(x0, y0, x2, y2);
    }
    else{
    imgproc.linesImg.line(x0, y0, x3, y3);
    }
    }
    else {
    if (x1 > 0) {
    if (y2 > 0){
    imgproc.linesImg.line(x1, y1, x2, y2);
    }
    else{
    imgproc.linesImg.line(x1, y1, x3, y3);
    }
    }
    else{
    imgproc.linesImg.line(x2, y2, x3, y3);
    }
    }
}
