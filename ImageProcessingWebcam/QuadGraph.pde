...
List<PVector> drawQuads(List<PVector> lines){
  ...
  for (int[] quad : this.findCycles()) {
    ...
    //because we draw all the results on one window, we want the lines to be drawn only on 
    //the first picture and not to be seen on the next ones
    //so we draw them separately on a surface of size of the original picture
    
    //either this or clear() below
    linesImg = createGraphics(640, 480);
...
Boolean checkQuad(PVector c1, PVector c2, PVector c3, PVector c4){
  ...
  //PROPER VALUES????????? for valid area?
  
    return isConvex(c1, c2, c3, c4) && validArea(c1, c2, c3, c4, 1000000, 1000) && nonFlatQuad(c1, c2, c3, c4);
